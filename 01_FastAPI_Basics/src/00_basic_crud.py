...
def get_employee_info(
    ...
    if q:
        return {
            "status": f"{info_str} of employee with {employee_id=} queried with parameters {q}"
        }
    return {
        "status": f"{info_str} of employee with {employee_id=} queried without parameters"
    }
...
def get_employee_info_adv(
    ...
    in_office: bool = Query(
        False, description="If person is in office or on vacation."
    ),
):
    if age:
        if in_office:
    ...
    if in_office:
        return {"status": "get any employees in office"}
    return {"status": "get any employees"}
