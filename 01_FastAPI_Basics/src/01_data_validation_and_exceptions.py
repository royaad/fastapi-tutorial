...
class SearchParams(BaseModel):
    ...
    def do_basic_checks(cls, values):
        ...
        if (first_name is None) != (last_name is None):
            raise ValueError("Both first_name and last_name must be provided together or omitted.")
...
def create_employee(employee: Employee):
    ...
    for employee_id, existing_employee in employee_db.items():
        if (
            existing_employee["first_name"].lower() == employee.first_name.strip().lower()
            and existing_employee["last_name"].lower() == employee.last_name.strip().lower()
    ...
    matching_employees = [
        {
            "id": emp_id,
            **employee,
            "age": today.year - datetime.strptime(employee["birthday"], "%Y-%m-%d").year
        }
        for emp_id, employee in employee_db.items()
        if age_query.min_age <= today.year - datetime.strptime(employee["birthday"], "%Y-%m-%d").year <= age_query.max_age
    ]

    if not matching_employees:
        raise HTTPException(status_code=404, detail="No employees found in the specified age range")
...
def get_employee_by_name(name_query: Annotated[NameParams, Query()]):
    ...
    matching_employees = [
        ...
        for emp_id, employee in employee_db.items()
        if name_query.first_name == employee["first_name"] and name_query.last_name == employee["last_name"]
    ]
...
def get_employee_v0(
    ...
    matching_employees = []
    for emp_id, employee in employee_db.items():
        employee_age = today.year - datetime.strptime(employee["birthday"], "%Y-%m-%d").year
        age_match = (
            search_query.min_age is None or 
            (search_query.min_age <= employee_age <= search_query.max_age) # type: ignore
        )
        name_match = (
            search_query.first_name is None or 
            (employee["first_name"] == search_query.first_name and employee["last_name"] == search_query.last_name)
        )
...
def get_employee_v1(
    ...
    for emp_id, employee in employee_db.items():
        employee_age = today.year - datetime.strptime(employee["birthday"], "%Y-%m-%d").year
        age_match = (
            age_query.min_age is None or 
            (age_query.min_age <= employee_age <= age_query.max_age) # type: ignore
        )
        name_match = (
            name_query.first_name is None or 
            (employee["first_name"] == name_query.first_name and employee["last_name"] == name_query.last_name)
        )
        if age_match and name_match:
            matching_employees.append({"id": emp_id, **employee, "age": employee_age})
...
def bundle_name_and_age(
    ...
    if first_name is None or last_name is None:
        ...
        data.update(
            NameParams(first_name=first_name, last_name=last_name).model_dump()
        )
    ...
    if min_age is None or max_age is None:
        ...
        data.update(
            AgeParams(min_age=min_age, max_age=max_age).model_dump()
        )
...
def get_employee_v2(
    ...
    for emp_id, employee in employee_db.items():
        employee_age = today.year - datetime.strptime(employee["birthday"], "%Y-%m-%d").year
        age_match = (
            search_query is None or search_query.min_age is None or
            (search_query.min_age <= employee_age <= search_query.max_age)
        )
        name_match = (
            search_query is None or search_query.first_name is None or
            (employee["first_name"] == search_query.first_name and employee["last_name"] == search_query.last_name)
        )
        if age_match and name_match:
            matching_employees.append({"id": emp_id, **employee, "age": employee_age})
